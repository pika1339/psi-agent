...
- **live 反馈（`PSI_FEISHU_LIVE_FEEDBACK`，默认开，显式设 `0` 才关）**：卡片上实时显示「思考 + 工具调用」的时间线。**默认从关翻成开是拍过板的**——「工具跑几十秒时卡片一动不动」是默认路径下最常见的抱怨，让默认就是好的那条，而不是靠每处部署记得导出一次环境变量。关的判据是**显式的 `0`** 而非「非 `1`」：后者会让 `true`/`yes` 这类拼法静默关掉 live，而写这些值的人意思明显是要开。**卡片只有一个 markdown 元素**（`MarkdownStreamController` 的 spec 把 `elements` 硬编码成单个 markdown），状态行、过程块、正文共用同一个 `_content`，没有第二个元素可以放过程；而 `append` 的 `merge_streaming_text` 去重在 prev 含过程块时会吃掉正文开头的字。故 live **放弃 SDK 的 `append` 打字机**，每 `RENDER_INTERVAL_SECONDS`（0.7s）用 `set_content` 整块重写。**打字机变成 0.7 秒一跳是已拍板的取舍**，勿"优化"回 `append`。「搬到独立元素 / 折叠面板」要换 `CardStreamController`（整卡 JSON + `patch_card`），那条路没有流式打字机，属产品取舍、已否掉。
