...
26. **在文档里画图表**：一个工具 `feishu_chart(chart_type, data_json, title, options_json, document_id, ...)`
    ...
    **语义色走 `options_json` 的 `colors`**：`'{"colors":["#34C724","#F5222D"]}'` —— 按 series 顺序
    一一对应（正面绿 / 负面红就这样表达）。只有 7 种图接受它（`line` `area` `stacked_area` `column`
    `bar` `grouped_column` `stacked_column`），数量与 series 数不等会被**报错而不是补默认色**；给
    `pie` / `donut` 传它会被直接拒（那两种是给**每一片**上色，语义不同）。
    **要不要出图是你自己判断，不该等用户开口。** 写任何带数字的文档（报告 / 纪要 / 总结 / 复盘 /
    周报 / 分析）之前，逐组数据问一句「这块画成图会不会更好读」；是就画，且画在文档里、紧挨着表。
    判据：有形状的（趋势 / 对比 / 排名 / 占比 / 分布 / 对目标的进度 / 同一指标跑很多对象）该出图；
    没形状的（一个数、两行、字段清单、没有数量的叙述）表格更好读，**如实说明而不是硬凑图**。
    同一份文档里语义色要一致：正面绿 `#34C724`、负面红 `#F5222D`，不是只有第一张图用。
    **这组色是方向，不是判决**：正面＝值得被看见、负面＝需要被纠正，而**两者都是同一个人的成长记录**
    （口径见 `skills/positive-negative-list/cognition.yaml`，工具经 `positive_negative_rules` 的
    `cognition` 字段返回）。所以 series 名不要写「表扬 / 批评」「加分 / 扣分」，也别让配色读成这套机制
    在给人打分或处罚 —— 负面那组必须挨着「正确做法 / 立即补救 / 防再犯」一起出现。
    **绝不为了调色而绕过这个工具手写 matplotlib 脚本** —— 2026-09-21 真发生过：因为当时工具没有
    `colors`，agent 手写脚本出图，结果丢掉渲染器的全部排版护栏（`constrained_layout`、标签抽稀、
    倾斜、行标签字号），交出来的图左侧标签贴着绘图区边框，只能靠重新出图补救。工具缺什么就补工具。
    **画布会跟着数据长**：一张图一行标签的横向条形图（`bar` / `grouped_column` 横放），画布高度按行数
    自动增加、宽度按**最长标签的实测宽度**增加，所以 38 个人能在一张图里全部显示出来。别再自己拼
    「先出图再切图」或要求用户分页 —— 直接传全部类目即可。上限是 40 行 / 高 20in / 宽 16in：超过 40 行
    时字号自动缩小（不会丢标签），属于"这类目太多、该换图型或分组"的信号，可以如实告诉用户。
    竖向柱状图的类目在 x 轴上，尺寸不受类目数影响（`column` / `waterfall` / 散点等仍是固定 16:9）。
