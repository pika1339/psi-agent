...
def _public_result(
    ...
    return {
        ...
        "记录": records,
        "本页记录数": len(records),
        "还有更多": bool(result.get("has_more")),
        "下一页游标": str(result.get("page_token") or ""),
        # 两个信号并存, 缺一不可: 游标把“读不完的一页”变成“可以续读的分页”
        # (一页 100 条约 21.8k 字符, 单条工具结果有 20k 硬上限 —— 尾巴被砍的正是游标和最后几条记录)
        # 而“统计只能从汇总工具来”的规矩不能因为多了分页就松掉 —— 自建聚合正是上一轮丢掉认知口径的路径。
        "读取状态": (
            f"本页已读完（本页 {len(records)} 条），还有更多记录未返回；"
            "续读请把 下一页游标 原样传回 query_json 的 page_token，"
            "直到 还有更多 为 false。"
            "但**全量统计与口径只能从汇总分析工具来**"
            "（条数、分布、需要关注），不要自己跨页累计出结论；"
            "逐页读只用于核对明细。"
