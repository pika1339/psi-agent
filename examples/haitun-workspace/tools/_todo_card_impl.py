...
async def _apply_row_transition(
    *,
    ...
    Prefers the self-contained ``card_state`` embedded in the click (see module
    docstring) — rebuilds the WHOLE card from it, which keeps every remaining button's
    embedded state correct no matter which row gets clicked next. Falls back to the
    legacy single-row splice (``_rebuild_row_in_card`` against Feishu's own card
    snapshot) only when the click carries no ``card_state`` — i.e. a card sent before
    this existed. Returns a short status string for the caller to surface in its result
    (``"ok"``, or a ``"skipped_..."``/``"edit_failed: ..."`` reason) — never raises.
    """
    state_json = value.get("card_state")
    if isinstance(state_json, str) and state_json.strip():
        state = _parse_card_state(state_json)
        if state is None:
            return "skipped_invalid_card_state"
        rows = state.get("rows")
        if not isinstance(rows, list) or index >= len(rows) or not isinstance(rows[index], dict):
            return "skipped_row_index_out_of_range"
        rows[index] = {**rows[index], "done": new_done, "round": new_round}
        new_card = _build_card_from_state(state)
        # edit_card_impl syncs the Channel's own click-time snapshot to this new content
        # (see its docstring) — without that sync, every click after this one would find
        # a stale snapshot, fail its check, and degrade into a slow fetch-and-fallback
        # ("已提交" placeholder) instead of a fast, in-place update.
    ...
    if not isinstance(current_card, dict):
        return "skipped_missing_card_in_payload"
    item = {
    ...
    if rebuilt is None:
        return "skipped_row_slot_not_found"
    edit_result = await _f.edit_card_impl(message_id, json.dumps(rebuilt, ensure_ascii=False), user_key)
    return "ok" if edit_result.get("ok") else f"edit_failed: {edit_result}"


async def _sync_ledger_status(value: dict[str, Any], status: str) -> None:
    ...
    await _f.update_bitable_records_impl(app_token, table_id, records_json)
