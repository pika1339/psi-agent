...
| POST | `/feishu/route` | 幂等路由飞书会话到 Session：群聊按 chat_id（整群共用），私聊按 open_id（一人一个），首次按需 spawn。**channel 进程专用**：要带服务间签名，未签名 401 |
| GET | `/feishu/routes` | 列出飞书会话 → Session 路由（channel 进程专用，同一道签名判据） |
| GET | `/titles` | 获取所有会话标题 |
| POST | `/titles` | 设置会话标题 |
